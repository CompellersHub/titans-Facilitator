"use client"
import { Calendar, FileText, User, Edit, Trash2, Download, Users } from "lucide-react"
...
import { useAssignment, useAssignmentSubmissions } from "@/hooks/use-assignments"
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        <AlertDescription>Failed to load assignment details. Please try again.</AlertDescription>
      </Alert>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="details" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="submissions">Submissions {submissions && `(${submissions.length})`}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="details" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{format(dueDate, "MMM dd")}</div>
                <p className="text-xs text-muted-foreground">{format(dueDate, "yyyy 'at' HH:mm")}</p>
                {isOverdue && (
            ...
            <CardContent>
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{assignment.teacher.role}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {submissionsLoading ? (
            ...
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-muted-foreground">Students haven't submitted their assignments yet.</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Submitted {format(new Date(submission.submitted_at), "PPp")}</CardDescription>
          </div>
